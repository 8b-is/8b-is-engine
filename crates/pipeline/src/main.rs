...
async fn main() {
    ...
    match args.first().map(String::as_str) {
        ...
        Some("stage") => {
            ...
            let dst = std::path::Path::new(args.get(2).map(String::as_str).unwrap_or("assets/staged"));
            match pipeline::stager::stage_file(src, dst) {
                Ok(e) => println!("staged {} ({}B, sha256 {})", e.path, e.bytes, &e.sha256[..16]),
                Err(e) => {
...
async fn run(args: &[String], cfg: &PipelineConfig) {
    ...
    match run_pipeline(&text, cfg, with_art).await {
        Ok(out) => {
            ...
            println!("  gaia:    weather {} · gravity {} · memory {}", out.manifest.gaia.weather, out.manifest.gaia.gravity, out.manifest.gaia.memory);
            println!("  board:   {} tiles of {{-1,0,+1}}", out.manifest.board.len());
            println!("  fauna:   {}", out.manifest.archetypes.iter().map(|a| a.name.as_str()).collect::<Vec<_>>().join(", "));
            println!("  items:   {}", out.manifest.items.iter().map(|i| i.name.as_str()).collect::<Vec<_>>().join(", "));
            println!("  staged:  {} ({}B, sha256 {}…)", out.entry.path, out.entry.bytes, &out.entry.sha256[..12]);
