...
async fn main() {
    ...
    if let Some(i) = args.iter().position(|a| a == "--retire-after") {
        retire = args.get(i + 1).and_then(|v| v.parse().ok());
    }
    let mut node = new_node(&brief, mode, port);
    if let Some(t) = retire {
        node = node.with_retire(t);
    }
